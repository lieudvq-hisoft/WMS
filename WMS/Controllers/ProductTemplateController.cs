...
public class ProductTemplateController : ControllerBase
{
    ...
    {
        var result = await _productTemplateService.GetStockQuant(paginationModel, id);
        if (result.Succeed) return Ok(result.Data);
        return BadRequest(result.ErrorMessage);
    }

    [HttpGet("Select/{id}")]
    public async Task<ActionResult> GetProductVariantForSelect(Guid id)
    {
        var result = await _productTemplateService.GetProductVariantForSelect(id);
        if (result.Succeed) return Ok(result.Data);
        return BadRequest(result.ErrorMessage);
    }
    ...
    {
        var result = await _productTemplateService.DeleteImage(id);
        if (result.Succeed) return Ok(result.Data);
        return BadRequest(result.ErrorMessage);
    }
}